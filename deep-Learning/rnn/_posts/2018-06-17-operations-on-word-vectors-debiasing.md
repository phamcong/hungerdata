---
layout: post
title: "Operations on word vectors (embeddings) - Debiasing"
description: "Deeplearning.ai - Course RNN - Week 2 - Operations on word vectors"
categories: []
tags: [rnn-tutorial]
redirect_from:
  - /2018/06/17/
...
def cosine_similarity(u, v):
    ...
    dot = sum(u*v)
    # Compute the L2 norm of u (â‰ˆ1 line)
    norm_u = np.linalg.norm(u)
    
    # Compute the L2 norm of v (â‰ˆ1 line)
...
def neutralize(word, g, word_to_vec_map):
    ...
    # Compute e_biascomponent using the formula give above. (â‰ˆ 1 line)
    e_biascomponent = sum(e*g) / sum(g**2) * g
...
```
    ...
    cosine similarity between receptionist and g, after neutralizing:  1.08241630407e-16
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    mu_B = sum(mu * bias_axis) / sum(bias_axis**2) * bias_axis
    mu_orth = mu - mu_B

    # Step 4: Use equations (7) and (8) to compute e_w1B and e_w2B (â‰ˆ2 lines)
    e_w1B = sum(e_w1 * bias_axis) / sum(bias_axis**2) * bias_axis
    e_w2B = sum(e_w2 * bias_axis) / sum(bias_axis**2) * bias_axis
        
    # Step 5: Adjust the Bias part of e_w1B and e_w2B using the formulas (9) and (10) given above (â‰ˆ2 lines)
    corrected_e_w1B = np.sqrt(np.abs(1 - sum(mu_orth**2))) * (e_w1B - mu_B) / np.linalg.norm((e_w1 - mu_orth) - mu_B)
    corrected_e_w2B = np.sqrt(np.abs(1 - sum(mu_orth**2))) * (e_w2B - mu_B) / np.linalg.norm((e_w2 - mu_orth) - mu_B)
